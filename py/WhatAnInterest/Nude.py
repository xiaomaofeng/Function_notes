...
class Nude(object):
    ...
    def parse(self):
        ...
        for y in range(self.height):
            for x in range(self.width):
                ...
                # judge whether current pixel is the skin pixel
                isSkin = True if self._classify_skin(r, g, b) else False
                # allocate id for every pixel
                ...
                for index in check_indexes:
                    ...
                    if self.skin_map[index].skin:
                        if (self.skin_map[index].region != None and region != None and region != -1 and self.skin_map[
                            index].region != region and self.last_from != region and self.last_to != self.skin_map[index].region):
                ...
                self._add_merge(self.detected_regions, self.merge_regions)
                self._analyse_regions()
                return self

    def _classify_skin(self, r, g, b):
        # RGB determine
        rgb_classifier = r > 95 and g > 40 and g < 100 and b > 20 and max([r, g, b]) - min([r, g, b]) > 15 and abs(
            r - g) > 15 and r > g and r > b

        # determine by processde RGB
        nr, ng, nb = self._to_normalized(r, g, b)
        norm_rgb_classifier = nr / ng > 1.185 and float(r * b) / ((r + g + b) ** 2) > 0.107 and float(r * g) / (
                (r + g + b) ** 2) > 0.112
    ...
    def _merge(self, detected_regions, merge_regions):
        # new_detected_regions represent the list of skin
        ...
        for index, region in enumerate(merge_regions):
            ...
            self._clear_region(new_detected_regions)

    def _clear_region(self, detected_regions):
        # svae the region whick the number of pixels more than specified quantity
        for region in detected_regions:
            if len(region) > 30:
    ...
    def _analyse_regions(self):
        if len(self.skin_regions) < 3:
            self.message = "Less than 3 skin regions ({__skin_regions_size})".format(__skin_regions_size=len(self.skin_regions))
            self.result = False
            return self.result

        self.skin_regions = sorted(self.skin_regions, key=lambda s: len(s), reverse=True)

        # cal the total number of pixels of skin
        ...
        if len(self.skin_regions[0]) / self.total_pixels * 100 < 45:
    ...
    def showSkinRegions(self):
        ...
        # put skin pixel's id in skinIdSet
        for sr in self.skin_regions:
            for pixel in sr:
                skinIdSet.add(pixel.id)

        # set skin to white, other to black
